...
namespace SBUpdater.Manufacturers
{
    ...
    public class Stanley : Manufacturer
    {
        private readonly string _stanleyLink = "http://handtool.ru/producers/stanley.html?page=";
        private void ParseStanley(URLs url)
        {
            ...
            do
            {
                ...
                try
                {
                    html.LoadHtml(client.DownloadString(url.Url.Replace("В®", "%C2%AE").Replace("в„ў", "%E2%84%A2").Replace("В°", "%C2%B0")));
                    loaded = true;
                }
                catch (Exception ex) { }
            }
            ...
            var fileName = (@"Stanley\" + url.ProductName + ".png").Replace("\"", "").Replace("/", "").Replace("<", "").Replace(">", "").Replace(":", "");
            ...
            for (int i = 0; i < l; i++)
            {
                ...
                {
                    Description = descr,
                    Meta_Description = metaDescription.Replace("{0}", "Stanley " + name),
                    Meta_Keyword = metaKeywords.Replace("{0}", "Stanley " + name)
                };
                tools.Description = description;
                tools.Height = 1M;
                tools.Image = "data/" + fileName.Replace(@"\", "/");
                tools.Length = 1M;
                tools.Manufacturer_id = 16;
                tools.Model = name;
                tools.Price = 1M;
        ...
        private void ParseStanleyLinks(URLs url)
        {
            var html = new HtmlDocument();
            var client = new WebClient();
        ...
        public ICommand UpdateStanleyLinks
        {
            get
            {
                return new Command(() =>
                {
                    ...
                    do
                    {
                        ParseStanley(ProductUrls.First());
        ...
        public ICommand UpdateStanleyPrice
        {
            get
            {
                return new Command(() =>
                {
                    ...
                    if (dialog.ShowDialog() ?? false)
                    {
                        ...
                        for (int j = 772; j < 2162; j++)
                        {
                            var model = ((worksheet.Cells[j, 1] as Range).Text).ToString();
                            if (model == "")
                                continue;
                            var priceString = ((dynamic)(worksheet.Cells[j, 6] as Range).Text).ToString();
                        ...
                        {
                            string cmdText = "UPDATE `oc_product` SET\r\nprice = @price WHERE sku = @model and manufacturer_id = 16";
