...
{
    public class Stern : Manufacturer
    {
        private readonly string _instLink = "http://instrument.ru";
        private void ParseStern(URLs url)
        {
            ...
            try
            {
                ...
                var fileName = (@"Stern\" + sku + ".png").Replace("\"", "").Replace("/", "").Replace("<", "").Replace(">", "").Replace(":", "");
                ...
                name = name.Replace("Stern", "").TrimEnd(new char[] { ' ', ',', '.', '|' });
                ...
                {
                    Description = descr,
                    Meta_Description = metaDescription.Replace("{0}", "Stern " + name + " " + url.ProductName),
                    Meta_Keyword = metaKeywords.Replace("{0}", "Stern " + name + " " + url.ProductName)
                };
                tools.Description = description;
                tools.Height = 1M;
                tools.Image = "data/" + fileName.Replace(@"\", "/");
                tools.Length = 1M;
                tools.Manufacturer_id = 38;
                tools.Model = sku;
                tools.Name = name;
                tools.Price = 1M;
                tools.Sku = sku;
        ...
        private void ParseSternLinks(URLs url)
        {
            var html = new HtmlDocument();
            var client = new WebClient();
        ...
        public ICommand UpdateSternLinks
        {
            get
            {
                return new Command(() =>
                {
                    ...
                    {
                        if (i == 1)
                            ParseSternLinks(new URLs
                            {
                                CategoryName = "",
                                Url = "http://instrument.ru/catalog/brands/Stern/",
                            });
                        else
                            ParseSternLinks(new URLs
                            {
                                CategoryName = "",
                                Url = "http://instrument.ru/catalog/brands/Stern/page" + i + "/",
                            });
                    }
                    string queryString = "SELECT model\r\nFROM oc_product\r\nWHERE manufacturer_id = 38";
                    Connection.Open();
        ...
        public ICommand UpdateSternPrice
        {
            get
            {
                return new Command(() =>
                {
                    ...
                    string queryString = "SELECT sku\r\nFROM oc_product\r\nWHERE manufacturer_id > 25 and manufacturer_id<38";
                    Connection.Open();
                    List<string> products = new List<string>();
                    MySqlDataReader reader = LoadFromDb(queryString);
                    while (reader.Read())
                    ...
                    if (dialog.ShowDialog() ?? false)
                    {
                        ...
                        foreach (Tools tools2 in list)
                        {
                            ...
                            {
                                string cmdText = "UPDATE `oc_product` SET\r\nprice = @price, status=TRUE WHERE sku = @model and manufacturer_id > 25 and manufacturer_id<38";
