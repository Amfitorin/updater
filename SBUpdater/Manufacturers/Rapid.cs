...
{
    public class Rapid : Manufacturer
    {
        private readonly string _rapidLink = "http://handtool.ru/producers/rapid.html?page=";
        private void ParseRapid(URLs url)
        {
            ...
            var fileName = (@"Rapid\" + url.ProductName + ".png").Replace("\"", "").Replace("/", "");
            ...
            for (int i = 0; i < l; i++)
            {
                ...
                {
                    Description = descr,
                    Meta_Description = metaDescription.Replace("{0}", "Rapid " + name),
                    Meta_Keyword = metaKeywords.Replace("{0}", "Rapid " + name)
                };
                tools.Description = description;
                tools.Height = 1M;
                tools.Image = "data/" + fileName.Replace(@"\", "/");
                tools.Length = 1M;
                tools.Manufacturer_id = 20;
                tools.Model = name;
                tools.Price = 1M;
        ...
        private void ParseRapidLinks(URLs url)
        {
            var html = new HtmlDocument();
            var client = new WebClient();
            html.LoadHtml(client.DownloadString(url.Url));
        ...
        public ICommand UpdateRapidLinks
        {
            get
            {
                return new Command(() =>
                {
                    for (int i = 1; i < 4; i++)
                    {
                        ParseRapidLinks(new URLs
                        {
                            CategoryName = "",
                            Url = _rapidLink + i,
                        });
                    }
                    string queryString = "SELECT model\r\nFROM oc_product\r\nWHERE manufacturer_id = 20";
                    Connection.Open();
        ...
        public ICommand UpdateRapidPrice
        {
            get
            {
                return new Command(() =>
                {
                    ...
                    if (dialog.ShowDialog() ?? false)
                    {
                        ...
                        {
                            string cmdText = "UPDATE `oc_product` SET\r\nprice = @price WHERE sku = @sku and manufacturer_id = 20";
