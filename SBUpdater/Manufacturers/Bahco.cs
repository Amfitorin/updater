...
{
    public class Bahco : Manufacturer
    {
        private readonly string _bahcoLink = "http://handtool.ru/producers/bahco.html?page=";
        private void ParseBahco(URLs url)
        {
            ...
            var fileName = (@"Bahco\" + url.ProductName + ".png").Replace("\"", "").Replace("/", "");
            ...
            for (int i = 0; i < l; i++)
            {
                ...
                {
                    Description = descr,
                    Meta_Description = metaDescription.Replace("{0}", "Bahco " + name),
                    Meta_Keyword = metaKeywords.Replace("{0}", "Bahco " + name)
                };
                tools.Description = description;
                tools.Height = 1M;
                tools.Image = "data/" + fileName.Replace(@"\", "/");
                tools.Length = 1M;
                tools.Manufacturer_id = 18;
                tools.Model = name;
                tools.Price = 1M;
        ...
        private void ParseBahcoLinks(URLs url)
        {
            var html = new HtmlDocument();
            var client = new WebClient();
            html.LoadHtml(client.DownloadString(url.Url));
        ...
        public ICommand UpdateBahcoLinks
        {
            get
            {
                return new Command(() =>
                {
                    for (int i = 1; i < 20; i++)
                    {
                        ParseBahcoLinks(new URLs
                        {
                            CategoryName = "",
                            Url = _bahcoLink + i,
                        });
                    }
                    string queryString = "SELECT model\r\nFROM oc_product\r\nWHERE manufacturer_id = 18";
                    Connection.Open();
        ...
        public ICommand UpdateBahcoPrice
        {
            get
            {
                return new Command(() =>
                {
                    ...
                    if (dialog.ShowDialog() ?? false)
                    {
                        ...
                        {
                            string cmdText = "UPDATE `oc_product` SET\r\nprice = @price WHERE sku = @sku and manufacturer_id = 18";
