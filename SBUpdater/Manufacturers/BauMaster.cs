...
{
    public class BauMaster : Manufacturer
    {
        private void ParseBauMaster(URLs url)
        {
            ...
            try
            {
                ...
                if (documentNode.SelectSingleNode("//img[@class='main']") != null)
                {
                    ...
                    fileName = (@"BauMaster\" + url.ProductName + ".jpeg").Replace("\"", "").Replace("/", "").Replace("<", "").Replace(">", "").Replace(":", "");
                ...
                if (name.Contains(" BauMaster"))
                    name = name.Remove(name.IndexOf(" BauMaster"));
                else
                ...
                {
                    Description = descr,
                    Meta_Description = metaDescription.Replace("{0}", "BauMaster " + name + " " + url.ProductName),
                    Meta_Keyword = metaKeywords.Replace("{0}", "BauMaster " + name + " " + url.ProductName)
                };
                tools.Description = description;
                tools.Height = 1M;
                tools.Image = "data/" + fileName.Replace(@"\", "/");
                tools.Length = 1M;
                tools.Manufacturer_id = 27;
        ...
        private void ParseBauMasterLinks(URLs url)
        {
            var html = new HtmlDocument();
            var client = new WebClient();
            html.LoadHtml(client.DownloadString(url.Url));
        ...
        public ICommand UpdateBauMasterLinks
        {
            get
            {
                return new Command(() =>
                {
                    for (int i = 1; i < 4; i++)
                    {
                        ParseBauMasterLinks(new URLs
                        {
                            CategoryName = "Электроинструмент",
                            Url = "http://otvertka.ru/catalog/electroinstrument/?page=" + i + "&brand=26202",
                        });
                    }
                    for (int i = 1; i < 3; i++)
                    {
                        ParseBauMasterLinks(new URLs
                        {
                            ...
                            Url = "http://otvertka.ru/catalog/silovaja-texnika/?page=" + i + "&brand=26202",
                        });
                    }
                    for (int i = 1; i < 2; i++)
                    {
                        ParseBauMasterLinks(new URLs
                        {
                            ...
                            Url = "http://otvertka.ru/catalog/svarochnoe-oborudovanie/?page=" + i + "&brand=26202",
                        });
                    }
                    for (int i = 1; i < 2; i++)
                    {
                        ParseBauMasterLinks(new URLs
                        {
                            ...
                            Url = "http://otvertka.ru/catalog/sadovo-dachnaja/?page=" + i + "&brand=26202",
                        });
                    }
                    for (int i = 1; i < 2; i++)
                    {
                        ...
                        {
                            CategoryName = "klimat",
                            Url = "http://otvertka.ru/catalog/klimat/?page=" + i + "&brand=26202",
                        });
                    }
                    for (int i = 1; i < 2; i++)
                    {
                        ...
                        {
                            CategoryName = "stanki",
                            Url = "http://otvertka.ru/catalog/stanki/?page=" + i + "&brand=26202",
                        });
                    }
                    for (int i = 1; i < 2; i++)
                    {
                        ParseBauMasterLinks(new URLs
                        {
                            ...
                            Url = "http://otvertka.ru/catalog/sroy-oborudovanie/?page=" + i + "&brand=26202",
                        });
                    }
                    for (int i = 1; i < 2; i++)
                    {
                        ParseBauMasterLinks(new URLs
                        {
                            ...
                            Url = "http://otvertka.ru/catalog/nasosnoe-oborudovanie/?page=" + i + "&brand=26202",
                        });
                    }
                    for (int i = 1; i < 2; i++)
                    {
                        ParseBauMasterLinks(new URLs
                        {
                            ...
                            Url = "http://otvertka.ru/catalog/ruchnoiinstrument/?page=" + i + "&brand=26202",
                        });
                    }
                    ...
                    do
                    {
                        ParseBauMaster(ProductUrls.First());
