...
{
    public class Soyuz : Manufacturer
    {
        private void ParseSoyuz(URLs url)
        {
            ...
            try
            {
                ...
                if (documentNode.SelectSingleNode("//img[@class='main']") != null)
                {
                    ...
                    fileName = (@"Soyuz\" + url.ProductName + ".jpeg").Replace("\"", "").Replace("/", "").Replace("<", "").Replace(">", "").Replace(":", "");
                ...
                if (name.Contains(" СОЮЗ"))
                    name = name.Remove(name.IndexOf(" СОЮЗ"));
                else
                ...
                {
                    Description = descr,
                    Meta_Description = metaDescription.Replace("{0}", "Soyuz " + name + " " + url.ProductName),
                    Meta_Keyword = metaKeywords.Replace("{0}", "Soyuz " + name + " " + url.ProductName)
                };
                tools.Description = description;
                tools.Height = 1M;
                tools.Image = "data/" + fileName.Replace(@"\", "/");
                tools.Length = 1M;
                tools.Manufacturer_id = 28;
        ...
        private void ParseSoyuzLinks(URLs url)
        {
            var html = new HtmlDocument();
            var client = new WebClient();
            html.LoadHtml(client.DownloadString(url.Url));
        ...
        public ICommand UpdateSoyuzLinks
        {
            get
            {
                return new Command(() =>
                {
                    for (int i = 1; i < 5; i++)
                    {
                        ParseSoyuzLinks(new URLs
                        {
                            CategoryName = "Электроинструмент",
                            Url = "http://otvertka.ru/catalog/electroinstrument/?page=" + i + "&brand=50228",
                        });
                    }
                    for (int i = 1; i < 3; i++)
                    {
                        ParseSoyuzLinks(new URLs
                        {
                            ...
                            Url = "http://otvertka.ru/catalog/silovaja-texnika/?page=" + i + "&brand=50228",
                        });
                    }
                    for (int i = 1; i < 3; i++)
                    {
                        ParseSoyuzLinks(new URLs
                        {
                            ...
                            Url = "http://otvertka.ru/catalog/svarochnoe-oborudovanie/?page=" + i + "&brand=50228",
                        });
                    }
                    for (int i = 1; i < 5; i++)
                    {
                        ParseSoyuzLinks(new URLs
                        {
                            ...
                            Url = "http://otvertka.ru/catalog/sadovo-dachnaja/?page=" + i + "&brand=50228",
                        });
                    }
                    for (int i = 1; i < 2; i++)
                    {
                        ...
                        {
                            CategoryName = "klimat",
                            Url = "http://otvertka.ru/catalog/klimat/?page=" + i + "&brand=50228",
                        });
                    }
                    for (int i = 1; i < 2; i++)
                    {
                        ...
                        {
                            CategoryName = "stanki",
                            Url = "http://otvertka.ru/catalog/stanki/?page=" + i + "&brand=50228",
                        });
                    }
                    for (int i = 1; i < 2; i++)
                    {
                        ParseSoyuzLinks(new URLs
                        {
                            ...
                            Url = "http://otvertka.ru/catalog/sroy-oborudovanie/?page=" + i + "&brand=50228",
                        });
                    }
                    for (int i = 1; i < 2; i++)
                    {
                        ParseSoyuzLinks(new URLs
                        {
                            ...
                            Url = "http://otvertka.ru/catalog/nasosnoe-oborudovanie/?page=" + i + "&brand=50228",
                        });
                    }
                    for (int i = 1; i < 2; i++)
                    {
                        ParseSoyuzLinks(new URLs
                        {
                            ...
                            Url = "http://otvertka.ru/catalog/ruchnoiinstrument/?page=" + i + "&brand=50228",
                        });
                    }
                    for (int i = 1; i < 2; i++)
                    {
                        ParseSoyuzLinks(new URLs
                        {
                            ...
                            Url = "http://otvertka.ru/catalog/izmiritilniy-instrument/?page=" + i + "&brand=50228",
                        });
                    }
                    string queryString = "SELECT model\r\nFROM oc_product\r\nWHERE manufacturer_id = 28";
                    Connection.Open();
