...
{
    public class Palisad : Manufacturer
    {
        private readonly string _instLink = "http://instrument.ru";

        private void ParsePalisad(URLs url)
        {
            ...
            try
            {
                ...
                var fileName = (@"Palisad\" + sku + ".png").Replace("\"", "").Replace("/", "").Replace("<", "").Replace(">", "").Replace(":", "");
                ...
                {
                    Description = descr,
                    Meta_Description = metaDescription.Replace("{0}", "Palisad " + name + " " + url.ProductName),
                    Meta_Keyword = metaKeywords.Replace("{0}", "Palisad " + name + " " + url.ProductName)
                };
                tools.Description = description;
                tools.Height = 1M;
                tools.Image = "data/" + fileName.Replace(@"\", "/");
                tools.Length = 1M;
                tools.Manufacturer_id = 33;
                tools.Model = sku;
                tools.Name = name;
                tools.Price = 1M;
                tools.Sku = sku;
        ...
        private void ParsePalisadLinks(URLs url)
        {
            var html = new HtmlDocument();
            var client = new WebClient();
        ...
        public ICommand UpdatePalisadLinks
        {
            get
            {
                return new Command(() =>
                {
                    ...
                    {
                        if (i == 1)
                            ParsePalisadLinks(new URLs
                            {
                                CategoryName = "",
                                Url = "http://instrument.ru/catalog/brands/Palisad/",
                            });
                        else
                            ParsePalisadLinks(new URLs
                            {
                                CategoryName = "",
                                Url = "http://instrument.ru/catalog/brands/Palisad/page" + i + "/",
                            });
                    }
                    string queryString = "SELECT model\r\nFROM oc_product\r\nWHERE manufacturer_id = 33";
                    Connection.Open();
                    ...
                    do
                    {
                        ParsePalisad(ProductUrls.First());
        ...
        public ICommand UpdatePalisadPrice
        {
            get
            {
                return new Command(() =>
                {
                    ...
                    string queryString = "SELECT sku\r\nFROM oc_product\r\nWHERE manufacturer_id > 25 and manufacturer_id<33";
                    Connection.Open();
                    List<string> products = new List<string>();
                    MySqlDataReader reader = LoadFromDb(queryString);
                    while (reader.Read())
                    ...
                    if (dialog.ShowDialog() ?? false)
                    {
                        ...
                        foreach (Tools tools2 in list)
                        {
                            ...
                            {
                                string cmdText = "UPDATE `oc_product` SET\r\nprice = @price, status=TRUE WHERE sku = @model and manufacturer_id > 25 and manufacturer_id<33";
