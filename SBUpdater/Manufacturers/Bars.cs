...
{
    public class Bars : Manufacturer
    {
        private readonly string _instLink = "http://instrument.ru";
        private void ParseBars(URLs url)
        {
            ...
            try
            {
                ...
                var fileName = (@"Bars\" + sku + ".png").Replace("\"", "").Replace("/", "").Replace("<", "").Replace(">", "").Replace(":", "");
                ...
                {
                    Description = descr,
                    Meta_Description = metaDescription.Replace("{0}", "Bars " + name + " " + url.ProductName),
                    Meta_Keyword = metaKeywords.Replace("{0}", "Bars " + name + " " + url.ProductName)
                };
        ...
        private void ParseBarsLinks(URLs url)
        {
            var html = new HtmlDocument();
            var client = new WebClient();
        ...
        public ICommand UpdateBarsLinks
        {
            get
            {
                return new Command(() =>
                {
                    ...
                    {
                        if (i == 1)
                            ParseBarsLinks(new URLs
                            {
                                CategoryName = "",
                                Url = "http://instrument.ru/catalog/brands/Bars/",
                            });
                        else
                            ParseBarsLinks(new URLs
                            {
                                CategoryName = "",
                                Url = "http://instrument.ru/catalog/brands/Bars/page" + i + "/",
                            });
                    }
                    string queryString = "SELECT model\r\nFROM oc_product\r\nWHERE manufacturer_id = 34";
                    Connection.Open();
        ...
        public ICommand UpdateBarsPrice
        {
            get
            {
                return new Command(() =>
