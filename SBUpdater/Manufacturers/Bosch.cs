...
{
    public class Bosch : Manufacturer
    {

        private readonly string _link = "http://www.bosch-professional.com/ru/ru/";
        private void ParseBlueBosch(URLs url)
        {
            ...
            var fileName = (@"Bosch\" + url.ProductName + ".jpg").Replace("/", " ").Replace("*", " ");
            ...
            var kompl = html.GetElementbyId("ct_1").SelectSingleNode(".//ul") == null ? "" : html.GetElementbyId("ct_1").SelectSingleNode(".//ul").OuterHtml;
        ...
        private void ParseBlueBoschLinks(URLs url)
        {
            HtmlNode node2;
        ...
        public ICommand UpdateBoschLinks
        {
            get
            {
                return new Command(() =>
                {
                    ...
                    do
                    {
                        ParseBlueBoschLinks(CategoryUrls.First<URLs>());
                        CategoryUrls.RemoveAt(0);
                    }
                    while (CategoryUrls.Count > 0);
                    string queryString = "SELECT model\r\nFROM oc_product where manufacturer_id=11";
                    Connection.Open();
                    ...
                    do
                    {
                        ParseBlueBosch(ProductUrls.First<URLs>());
        ...
        public ICommand UpdateBlueBoschPrice
        {
            get
            {
                return new Command(() =>
                {
                    ...
                    if (dialog.ShowDialog() ?? false)
                    {
                        ...
                        for (int j = 14; j < 452; j++)
                        {
                            var sku = (string)((worksheet.Cells[j, 4] as Range).Text).ToString();
                            if (sku == "")
                                continue;
                            sku = sku.Insert(1, " ").Insert(5, " ").Insert(9, " ");
                            var price = decimal.Parse((worksheet.Cells[j, 7] as Range).Text);
                            var item = new Tools
                            {
                                Price = price,
                                Sku = sku,
                        ...
                        {
                            var cmdText = "UPDATE `oc_product` SET\r\nprice = @price WHERE sku = @sku and manufacturer_id = 11";
