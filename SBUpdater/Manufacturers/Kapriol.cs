...
{
    public class Kapriol : Manufacturer
    {
        private readonly string _kapriolLink = "http://handtool.ru/producers/kapriol.html?page=";
        private void ParseKapriol(URLs url)
        {
            ...
            var fileName = (@"Kapriol\" + url.ProductName + ".png").Replace("\"", "").Replace("/", "");
            ...
            for (int i = 0; i < l; i++)
            {
                ...
                {
                    Description = descr,
                    Meta_Description = metaDescription.Replace("{0}", "Kapriol " + name),
                    Meta_Keyword = metaKeywords.Replace("{0}", "Kapriol " + name)
                };
                tools.Description = description;
                tools.Height = 1M;
                tools.Image = "data/" + fileName.Replace(@"\", "/");
                tools.Length = 1M;
                tools.Manufacturer_id = 19;
                tools.Model = name;
                tools.Price = 1M;
        ...
        private void ParseKapriolLinks(URLs url)
        {
            var html = new HtmlDocument();
            var client = new WebClient();
            html.LoadHtml(client.DownloadString(url.Url));
        ...
        public ICommand UpdateKapriolLinks
        {
            get
            {
                return new Command(() =>
                {
                    for (int i = 1; i < 10; i++)
                    {
                        ParseKapriolLinks(new URLs
                        {
                            CategoryName = "",
                            Url = _kapriolLink + i,
                        });
                    }
                    string queryString = "SELECT model\r\nFROM oc_product\r\nWHERE manufacturer_id = 19";
                    Connection.Open();
                    ...
                    do
                    {
                        ParseKapriol(ProductUrls.First());
        ...
        public ICommand UpdateKapriolPrice
        {
            get
            {
                return new Command(() =>
                {
                    ...
                    if (dialog.ShowDialog() ?? false)
                    {
                        ...
                        {
                            string cmdText = "UPDATE `oc_product` SET\r\nprice = @price WHERE sku = @sku and manufacturer_id = 19";
